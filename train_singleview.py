...
def main_run(config_file):
    ...
    for r in range(runs):
        ...
        outputs_tr = method.transform(_to_loader(views_tr, batch_size=BS, train=False), output=True, out_norm=True, **config_file)
        outputs_te = method.transform(_to_loader(views_te, batch_size=BS, train=False), output=True, out_norm=True, **config_file)
