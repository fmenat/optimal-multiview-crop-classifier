...
def classification_metric(
    ...
    for r in range(R):
        ...
        if len(d_me.n_samples) == 2:
            ...
            plot_prob_dist_bin(ax[0,0], y_pred_prob, y_true_no_missing, f"(run-{r})")
...
def ensemble_avg(method_names, df_summary, df_std, data_tr,data_te,data_name, method="EnsembleAVG", **args):
...
def main_evaluation(config_file):
    ...
    if len(pool_names) != 0:
        for key_pool in pool_names:
            ensemble_avg(pool_names[key_pool], df_summary_sup, df_summary_sup_s, data_tr,data_te,data_name,
                    include_metrics=include_metrics,
                    plot_runs=config_file.get("plot_runs"),
                    dir_folder=output_dir_folder,
